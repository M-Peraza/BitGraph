...
int TestAnalyser::analyser(info_t* info){
	...
	if(info != nullptr){
		...
		if (bitgraph::all_equal(arrayOfAvSol))		{ info -> same_sol = true; }
		if (bitgraph::all_equal(arrayOfAvSteps))	{ info -> same_steps = true; }
		if (bitgraph::all_equal(arrayOfAvLB))		{ info -> same_lb = true; }
